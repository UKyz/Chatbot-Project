 ...
 * @returns {Array<String>} A sentence table
 */
const parseCsvFile = (data, delimiter, column) => R.pipeP(
  getFileContentAsString,
 ...
 * @param {String} delimiter (optional) The character that delimits the data
 *   for the csv
 * @param {String} column (optional) The column from which we want to retrieve
 *   the data for the csv
 * @returns {Array<String>} A sentence table
 */
const parseFile = (path, delimiter = ',', column = 'Log') => {
  return R.equals(R.slice(R.lastIndexOf('.', path), R.length(path), path),
    '.txt') ?
    parseTextFile(path) : parseCsvFile(path, delimiter, column);
};
